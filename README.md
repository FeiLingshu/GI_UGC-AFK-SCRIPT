...
- __~~使用 `GUID=**********` 关卡进行自动刷取奇域经验~~__
    - __由于大量速刷关卡遭清算，[我也制作了一个可以用于速刷的关卡](#my)，使用了一些技术手段规避千星奇域检测__
...
> - __关卡GUID：`24197625457` ~~`7017756273`~~__
> - __关卡人数限制：`8`__
> - __启用速刷功能需要满足以下条件__
>     - __单人游玩__
>     - __玩家位于房间的 `8` 号位中__
...
> [!WARNING]
> __关卡已被下架一次，后续可能会不定时主动移除当前关卡并重新上传，请自行查询新的关卡GUID__
> - __疑似遭举报（或速刷次数过多导致产生某种特征而被检测），请各位务必不要大肆宣传该关卡，以免被有心人蓄意破坏或大量速刷而被检测！关卡内存在一定体量的场景设计和可交互元素，光靠作者本人刷有效游玩无法100%规避检测！__
> - __另：提供一个备用的速刷关卡：`28492592753`，这个关卡需要在7号位才能进行速刷，切记__
...
> - __停止当前脚本/独立任务__
>
> - ___由于涉及到[自动更新](#WAIT)的相关异步调用，请不要在启动脚本后短时间内手动终止脚本，详情通过前述链接查看 ...___
>
> __推荐自己创建一个大厅/找一个没人的大厅，其他玩家头顶的昵称文本可能会影响OCR识别__

```
    前排提示：
    每个教程步骤均由无序列表项（开头带 · 的行）开始，到一张图像结束
    有些步骤的教程内容可能较多，不要看串了
```
...
- __移动至准备区，确认关卡中只有1位玩家，~~然后点击右上角「 收起 」按钮~~__

> [!IMPORTANT]
> __由于原神「 月之四 」版本中，可以在关卡结束后选择返回房间而不是返回大厅，后续点击「 收起 」按钮的步骤已不再被需要__

![](https://raw.githubusercontent.com/FeiLingshu/GI_UGC-AFK-SCRIPT/refs/heads/resources/2.png)

<a id="img3"></a>

- __~~最终在该界面时即可使用「 BetterGI 」运行脚本~~__

> [!IMPORTANT]
> __由于原神「 月之四 」版本中，可以在关卡结束后选择返回房间而不是返回大厅，最终界面应维持[上一张](#img2)图片的状态，而不是[这一张](#img3)__
...
> [!IMPORTANT]
> __由于原神「 月之四 」版本中，可以在关卡结束后选择返回房间而不是返回大厅，移除了进入房间的快捷键配置项__
...
> __请先确认：__
> - __进入关卡后，在关卡内耗时不超过60s__
> - __关卡结算界面呈现后，直到（由于点击 '返回房间' 按钮）离开关卡结算界面的过程，耗时不超过5s__
>    - __期间屏幕中央会显示 '正在进入关卡' 字样__
> - __从关卡结算界面离开后直至重新返回房间的过程，耗时不超过10s__
> - __重新返回房间后，直到（由于点击 '开始游戏' 按钮）离开房间界面的过程，耗时不超过5s__
>    - __期间屏幕中央会显示 '正在返回房间' 字样__
